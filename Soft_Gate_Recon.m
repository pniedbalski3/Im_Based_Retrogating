function Soft_Gate_Recon(fid,traj,Orig_ImSize,weights,write_path,FOV,Desired_ImSize,twix_obj,Patient_Name)
...
fid = fid(:,1:length(weights),:);
traj = traj(:,:,1:length(weights));

for i = 1:size(fid,2)
    fid(:,i,:) = fid(:,i,:)*weights(i);
end
...
tmpDCF = Recon.get_DCF(traj,Desired_ImSize,nIter);
SOS_Im = zeros(Desired_ImSize,Desired_ImSize,Desired_ImSize);
for j = 1:size(fid,3) %Loop over coils
    tmpfid = squeeze(fid(:,:,j));
    this_coil_Im = Recon.mem_eff_recon(Desired_ImSize,tmpfid,traj,tmpDCF,j,size(fid,3),[0 0 0],true);
...
niftiwrite(SOS_Im,fullfile(write_path,'Soft_Gated_Image'),Nifti_Info,'Compressed',true);
