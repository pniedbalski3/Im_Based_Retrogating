function SineParams=sineFit(x,y,fstart,varargin)
...
%PJN May 2021 - Edits to constrain frequency of fit
...
P2 = abs(Y/NumSamples);
P1 = P2(1:n2+1);
%PJN Addition - There's lots of things that could contribute to
%oscillations (mostly noise, artifacts, etc). 
%Try forcing the frequency to be within 1% of known HR - I can't think of a
%reason why different areas of the lungs would see a different frequency of
%RBC oscillations - just amplitude and phase. So, constraining the
%frequency to the heart rate should be a reasonable thing to do.

P1(2:end-1) = 2*P1(2:end-1);
fs = (0:n2)/n/T;% frequency scale
cutoff1 = find(fs>1.1*fstart,1,'first');
cutoff2 = find(fs<0.9*fstart,1,'last');
P1a = P1(cutoff2:cutoff1);
%fs = fs;%(1:cutoff);

% %FFT parameters at peak
[maxFFT,maxFFTindx]=max(P1a);%Peak magnitude and location
maxFFTindx = maxFFTindx+cutoff2;
