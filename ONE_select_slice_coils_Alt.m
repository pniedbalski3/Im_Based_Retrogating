function ONE_select_slice_coils_Alt(fid,traj,Orig_ImSize,Desired_Size,N_short)
...
trajr = [reshape(mytraj(1,:,:),1,[])',reshape(mytraj(2,:,:),1,[])',reshape(mytraj(3,:,:),1,[])'];


myDCF = Recon.get_DCF_Robertson(trajr,[1 1 1]*Desired_Size,20);

for i = 1:size(myfid,3)
    tmpfid = reshape(myfid(:,:,i),1,[])';
    Image(:,:,:,i+1) = myDCF.reconstruct(tmpfid,trajr);
    %Image(:,:,:,i+1) = Recon.mem_eff_recon(Desired_Size,myfid(:,:,i),mytraj,myDCF,i,size(myfid,3));
end
Image(:,:,:,1) = Tools.soscoilcombine(Image); 

imslice(abs(Image)/max(abs(Image(:))));
